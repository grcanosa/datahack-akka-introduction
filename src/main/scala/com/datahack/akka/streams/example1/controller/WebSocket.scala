...
class WebSocket()(
  ...
  implicit val executionContext: ExecutionContext) extends Directives {
  ...
  val queueActorRef: ActorRef = system.actorOf(Props(classOf[QueueActor], graph._1), "QueueActor")
  ...
  val websocketRoute: Route =
    path("websocket") {
      ...
      Flow.fromSinkAndSourceMat(printSink, graph._2)(Keep.both)
